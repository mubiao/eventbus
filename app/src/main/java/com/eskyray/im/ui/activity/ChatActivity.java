...
import com.eskyray.im.Constants;
import com.eskyray.im.R;
import com.eskyray.im.manager.ConnectionManager;
import com.eskyray.im.ui.adapter.ChatListAdapter;
import com.eskyray.im.ui.beans.IMessage;
import com.eskyray.im.ui.widget.RecordImageView;
...
public class ChatActivity extends BaseActivity implements View.OnClickListener, ChatMessageListener, ChatManagerListener, RecordImageView.OnRecordFinishedListener, FileTransferListener {
    ...
    public void mSend() {
        ...
        // 用户可以马上看到自己发送的消息
        IMessage localMsg = new IMessage(loggedUser, msg, TimeUtils.getTimeZoneDatabaseVersion(), IMessage.MESSAGE_TYPE_OUT);
        // 添加到消息列表, 更新ListView
        msgList.add(localMsg);
        adapter.notifyDataSetChanged();

        // 这条消息对于发送到的用户来说是接受的消息
        IMessage remoteMsg = new IMessage(loggedUser, msg,TimeUtils.getTimeZoneDatabaseVersion(), IMessage.MESSAGE_TYPE_IN);
    ...
    public void onFinished(File audioFile, int duration) {
        ...
        // localMsg
        IMessage localMsg = new IMessage(loggedUser, TimeUtils.getTimeZoneDatabaseVersion(), IMessage.MESSAGE_TYPE_OUT, duration, audioFile.getName());
        msgList.add(localMsg);
        adapter.notifyDataSetChanged();

        // remoteMsg
        IMessage remoteMsg = new IMessage(loggedUser, TimeUtils.getTimeZoneDatabaseVersion(), IMessage.MESSAGE_TYPE_IN, duration, audioFile.getName());
